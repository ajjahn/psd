...
/// Verify that a minimal PSD's layer and mask information section is parsed and the layer RGBA is
/// decoded as expected.
#[test]
...
/// Verify that a layer with a Cyrillic name can be looked up by name.
#[test]
...
/// Verify that a layer with a Chinese name can be looked up by name.
#[test]
...
/// Verify that Unicode layer names (including embedded NULs) are parsed consistently.
#[test]
...
/// Verify that clipping mask flags are parsed correctly for a fixture with clipped layers.
#[test]
...
/// Verify that a single group containing a single layer is parsed with correct parent IDs.
#[test]
...
/// Verify that layers inside and outside a group are assigned the correct parent IDs.
#[test]
...
/// Verify that multiple groups and their contained layers are parsed with stable IDs.
#[test]
...
// +-------------+-----+---------+
/// Verify that nested groups (group-in-group) are parsed with correct parent relationships.
#[test]
...
//
/// Verify that a complex group tree (multiple subgroups and mixed layers) is parsed with correct
/// parent IDs.
#[test]
...
/// Verify that calling `rgba()` on a layer larger than the PSD canvas does not panic.
#[test]
...
/// Verify that calling `rgba()` on an out-of-bounds layer does not panic.
#[test]
